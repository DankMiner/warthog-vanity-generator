...
VanitySearch::VanitySearch(Secp256K1 *secp, vector<std::string> &inputPrefixes,string seed,int searchMode,
  ...
  if (!hasPattern) {
    ...
    // Warthog wildcard search: addresses are 48 lowercase hex chars with no
    // version byte / HRP, so any hex char (0-9 a-f) can start a prefix.
    char c0 = inputPrefixes[0].data()[0];
    bool isHex = (c0 >= '0' && c0 <= '9') || (c0 >= 'a' && c0 <= 'f') || (c0 >= 'A' && c0 <= 'F');
    if (!isHex) {
      printf("Invalid start character '%c': Warthog addresses are hex (0-9 a-f).\n", c0);
      exit(1);
    }
    searchType = P2PKH;  // The only address scheme on Warthog.
...
// ----------------------------------------------------------------------------
// Warthog version of initPrefix.
//
// Input: a hex prefix (chars in [0-9a-fA-F]) of length 4..40, matched
//        against the canonical 48-char lowercase hex form of a Warthog
//        address. The first 40 chars are the HASH160 payload and are
//        the only chars searchable as a prefix; the trailing 8 chars are
//        a SHA-256 checksum derived from the payload.
//
// Output: fills PREFIX_ITEM with sPrefix (uint16_t = first 4 hex chars
//         packed as bytes h0[0],h0[1] — read on x86 as little-endian
//         (h0[1]<<8)|h0[0], same convention as `*(prefix_t*)hash160`),
//         the prefix string for the strncmp filter on suffix chars, and
//         the difficulty.
//
// Length < 4: refused here. The Python GUI / CLI auto-routes prefixes of
// length 1-3 to the CPU search instead — those are 1-65k-attempt searches
// that complete in milliseconds without needing the GPU at all.
bool VanitySearch::initPrefix(std::string &prefix, PREFIX_ITEM *it) {

  // Lowercase the prefix in-place — canonical Warthog form is lowercase.
  ...
  for (size_t i = 0; i < prefix.size(); i++) {
    ...
    if (!ok) {
      printf("Ignoring prefix \"%s\" (only hex chars 0-9 a-f allowed)\n",
             prefix.c_str());
      return false;
    }
  }
  ...
  if (prefix.length() < 4) {
    printf("Ignoring prefix \"%s\" (length<4 — for short prefixes use the "
           "CPU search; 4 hex chars is only ~65k attempts)\n",
           prefix.c_str());
    return false;
  }
  if (prefix.length() > 40) {
    printf("Ignoring prefix \"%s\" (length>40 — chars 41-48 are the SHA-256 "
           "checksum, not controllable as a vanity prefix)\n",
           prefix.c_str());
    return false;
  }

  if (searchType == -1) searchType = P2PKH;

  // Convert first 4 hex chars to 2 bytes for the lookup-table key.
  auto hexnib = [](char c) -> int {
    if (c >= '0' && c <= '9') return c - '0';
    if (c >= 'a' && c <= 'f') return c - 'a' + 10;
    return -1;
  };
  int n0 = hexnib(prefix[0]), n1 = hexnib(prefix[1]);
  int n2 = hexnib(prefix[2]), n3 = hexnib(prefix[3]);
  unsigned char b0 = (unsigned char)((n0 << 4) | n1);
  unsigned char b1 = (unsigned char)((n2 << 4) | n3);

  // sPrefix matches the convention used by `*(prefix_t*)hash160` on x86,
  // i.e. little-endian read of bytes {h0[0]=b0, h0[1]=b1}.
  it->sPrefix = (prefix_t)(((uint32_t)b1 << 8) | (uint32_t)b0);
  it->difficulty = pow(16.0, (double)prefix.length());
  it->isFull = (prefix.length() == 48);  // never reachable here (cap is 40)
  it->lPrefix = 0;
...
void VanitySearch::output(string addr,string pAddr,string pAddrHex) {
  ...
  if (startPubKeySpecified) {
    ...
    // Warthog has no WIF format; the wallet imports raw 32-byte hex via
    // `wart-wallet --restore <hex>`. Use `pAddr` (= GetPrivAddress output)
    // because it is padded to exactly 64 hex chars; `pAddrHex` from
    // Int::GetBase16() strips leading zero bytes and would yield 62/60
    // chars when the privkey starts with 0x00, which `parse_hex` in
    // Warthog rejects (see hex.cpp:83).
...
void VanitySearch::Search(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  while (isAlive(params)) {
    ...
    if (isAlive(params)) {
      ...
      fflush(stdout);  // CapStash patch: flush so piped readers see live status
    }

...
void Secp256K1::Check() {
  ...
  // Warthog build: the Bitcoin-format self-test addresses are gone — the
  // hex+SHA256-checksum encoding is unique to Warthog. Show a round-trip
  // for visibility instead.
  pub.x.SetBase16("3bf3d80f868fa33c6353012cb427e98b080452f19b5c1149ea2acfe4b7599739");
  pub.y = GetY(pub.x, false);
  printf("Check Calc PubKey (sample) %s :", GetAddress(P2PKH, true, pub).c_str());
  PrintResult(EC(pub));
...
std::string Secp256K1::GetPrivAddress(bool compressed, Int &privKey) {
  // Warthog: private keys are stored as raw 32-byte hex (no WIF, no
  // version byte, no checksum). The wart-wallet --restore command reads
  // exactly 64 lowercase hex chars. The `compressed` arg is ignored —
  // only compressed pubkeys are canonical on Warthog.
  static const char H[] = "0123456789abcdef";
  unsigned char k[32];
  privKey.Get32Bytes(k);  // big-endian 32 bytes
  char out[65];
  for (int i = 0; i < 32; i++) {
    out[2 * i]     = H[k[i] >> 4];
    out[2 * i + 1] = H[k[i] & 0xF];
...
// Warthog patch: helpers to render the 24-byte (20+4) Warthog address
// payload as 48 lowercase hex characters. There is no version byte, no
// Base58, no Bech32 — addresses are simply hex(hash160 || sha256(hash160)[0:4]).
// Mirrors warthog/src/shared/src/crypto/address.cpp:5-12.
//
// IMPORTANT: VanitySearch's `sha256_checksum` is *double* SHA-256
// (Bitcoin Base58Check). Warthog uses *single* SHA-256, so we invoke
// the plain `sha256(...)` from this same translation unit and take
// the first 4 bytes of the 32-byte output.

static inline void warthog_addr_to_hex(const unsigned char *hash20, char *out48) {
    static const char H[] = "0123456789abcdef";
    unsigned char buf[24];
    memcpy(buf, hash20, 20);
    unsigned char chk32[32];
    sha256((unsigned char *)hash20, 20, chk32);
    memcpy(buf + 20, chk32, 4);
    for (int i = 0; i < 24; i++) {
        out48[2 * i]     = H[buf[i] >> 4];
        out48[2 * i + 1] = H[buf[i] & 0xF];
    }
    out48[48] = 0;
}
...
std::vector<std::string> Secp256K1::GetAddress(int type, bool compressed, unsigned char *h1, unsigned char *h2, unsigned char *h3, unsigned char *h4) {
  // Warthog: type and compressed are accepted for upstream API parity, but
  // there is only one address scheme. SSE batch optimization for the
  // checksum is dropped — sha256_checksum on 20 bytes is a single block,
  // so calling it 4 times is the same throughput as the SSE batch on 25
  // bytes (each lane is also one block).
  std::vector<std::string> ret;
  char out[49];
  warthog_addr_to_hex(h1, out); ret.push_back(std::string(out));
  warthog_addr_to_hex(h2, out); ret.push_back(std::string(out));
  warthog_addr_to_hex(h3, out); ret.push_back(std::string(out));
  warthog_addr_to_hex(h4, out); ret.push_back(std::string(out));
  return ret;
}

std::string Secp256K1::GetAddress(int type, bool compressed, unsigned char *hash160) {
  // Warthog: type ignored, only one scheme. See warthog_addr_to_hex above.
  char out[49];
  warthog_addr_to_hex(hash160, out);
  return std::string(out);
}

std::string Secp256K1::GetAddress(int type, bool compressed, Point &pubKey) {
  // Warthog: only compressed pubkeys exist (33-byte, prefix 0x02/0x03).
  // GetHash160 always uses the P2PKH path (RIPEMD160(SHA256(pubkey))).
  unsigned char hash160[20];
  GetHash160(P2PKH, compressed, pubKey, hash160);
  char out[49];
  warthog_addr_to_hex(hash160, out);
  return std::string(out);
}
...
bool Secp256K1::CheckPudAddress(std::string address) {
  // Warthog: 48 hex chars, last 4 bytes = SHA-256(first 20 bytes)[0:4].
  if (address.size() != 48) return false;
  ...
  for (int i = 0; i < 24; i++) {
    ...
    int hi = hexval(address[2 * i]);
    int lo = hexval(address[2 * i + 1]);
    if (hi < 0 || lo < 0) return false;
    raw[i] = (unsigned char)((hi << 4) | lo);
  }
  unsigned char full[32];
  sha256(raw, 20, full);  // single SHA-256, take first 4 bytes
  return raw[20] == full[0] && raw[21] == full[1] &&
         raw[22] == full[2] && raw[23] == full[3];
}

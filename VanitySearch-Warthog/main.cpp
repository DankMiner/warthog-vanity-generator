...
#define RELEASE "Warthog-1.0.0 (DankMiner)"
...
void printUsage() {
  ...
  printf("VanitySearch-Warthog [-check] [-v] [-c] [-gpu] [-stop] [-i inputfile]\n");
...
void outputAdd(string outputFile, int addrType, string addr, string pAddr, string pAddrHex) {
  ...
  fprintf(f, "\nPubAddress: %s\n", addr.c_str());
  // Warthog: no WIF — wart-wallet imports raw hex via `--restore <hex>`.
  // Use pAddr (= GetPrivAddress, padded to 64 chars) not pAddrHex
  // (= Int::GetBase16, strips leading zero bytes — would break
  // `wart-wallet --restore` for keys with leading zeros).
...
int main(int argc, char* argv[]) {
  ...
  printf("GPU code not compiled, use -DWITHGPU when compiling.\n");
      ...
      Int k;
      bool isComp = true;
      // Warthog stores private keys as raw 64-char hex (no WIF prefix).
      k.SetBase16(argv[a]);
